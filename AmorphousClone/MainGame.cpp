...
void MainGame::init() {
	...
	// Sort sprites that are loaded currently.
	...
	// Init the staging manager, the camera for transforming pixel coordinates to UV coordinates, the game logic and the sprite batcher.
	...
	// Init the FPS manager for outputting FPS.
	_FPSManager.fpsinit();
}
...
void MainGame::gameLoop() {
	...
	// Runs this loop forever until the game state is set to exit.
	while (_gameState != GameState::EXIT) {
		// TODO: Add mutex when reading IOThreadState.
		...
		// IO thread is busy and we need to wait.
			...
			// Save the desired end state.
			desiredState = _gameState;

			// Set the state to the loading state in the meantime.
			_gameState = GameState::LOADING;
			currState = _gameState;

			// Reset the FPS accumulator as FPS is irrelevant currently.
			accumulator = 0;

			// Load the loading state and show it with the game logic.
			_StagingManager.loadState();
			_Game.getStage();
		...
		// TODO: Possible to use the FPS manager in the future?
		...
		// Process input no matter what so the thread doesn't hang.
...
void MainGame::renderGame() {
	...
	// Transform the coordinates based on the camera.
